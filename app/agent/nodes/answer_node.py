"""回答节点模块。
负责构建模型上下文、执行对话补全与工具调用，并持久化本轮输出。
当前阶段不负责知识库召回和 MCP 独立节点编排。
"""
...
class AnswerNode:
    ...
    async def run(self, state: AgentState) -> dict[str, object]:
        ...
        completion_result, executed_tool_calls = await self._execute_completion(
            prepared_context=prepared_context,
            execution_request=self._build_execution_request_from_state(state),
        )
        final_result = await self._persist_completion_result(
