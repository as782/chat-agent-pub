...
分类标准：
- policy：政策、制度、标准、规范、口径解释；也包括某类车辆、货物或人员是否允许进入/通行高速公路这类资格判断。
- route_planning：路线规划、从 A 到 B 怎么走、A 到 B 堵不堵、A 到 B 路况如何、出行方案；只要问题中能稳定识别出 A 到 B / A 去 B / A 回 B / A 至 B 这类 OD 结构，即使是在问拥堵、路况、是否好走，也优先归类为 route_planning。这个分类对应的是“起终点路线规划工具”，适合回答推荐路线、预计时长、里程、收费、沿途服务区等 OD 问题，不适合处理单个收费站归属。
- traffic_status：问单条或多条高速/路段/收费站/收费口/入口/出口/枢纽/互通/方向的路况、拥堵、封闭、施工、事故、实时状态，也包括“某收费站属于哪条高速/在哪条高速上/是哪个高速上的”这类道路归属问题。这个分类对应的是“道路事件聚合工具”，核心能力是按道路查询收费站、拥堵、管制、服务区、出入口事件。
- service_area：服务区、停车区、充电桩、加油站、商业配套、繁忙情况。这个分类只用于服务区/停车区设施详情，不要拿它回答收费站属于哪条高速。
- network_report：仅在用户明确要求全省/全网/路网汇总、报表、表格、对比、日报周报月报，或明显是在问整体路网概况时才使用。这个分类对应的是全网 topN/汇总工具，不适合回答单条路、单个收费站、单个服务区。
- general：普通问答、计算、时间、工具类问题

补充判断：
- 多个明确指定的高速/路段之间比较拥堵、车流量、事故或施工情况，优先归类为 traffic_status，而不是 network_report。
- 只有用户明确要求全省/全网/路网汇总、报表、表格、日报周报月报，且不是单纯 OD 路况或单点路况时，才优先归类为 network_report。
- 只要问题中能明确识别出起点和终点，例如“衢州到杭州走杭金衢高速堵不堵”“温州回杭州堵吗”，就优先归类为 route_planning，不要因为出现“堵不堵/路况/拥堵”等词改判成 traffic_status。
- 只要用户问的是单个收费站/收费口/入口/出口/枢纽/互通/方向，例如“佛堂收费站在哪条高速”“佛堂收费站堵吗”“诸暨北收费站温州方向出口正常吗”，都优先归类为 traffic_status，而不是 route_planning、service_area 或 network_report。
...
- “G60哪些服务区比较空”“沪昆高速有哪些服务区比较空”这类“道路 + 服务区”问题，本质仍然要落到 service_area；如需先识别道路，再进入服务区查询，不要误分到 traffic_status。
- 不要使用 service_area 去推断收费站属于哪条高速，也不要使用 network_report 去回答单个收费站/收费站方向的状态问题。
...
5. 只有在起点、终点等关键参数真正缺失且无法继续执行时，才标记 need_clarification=true；不要把 OD 场景里的 route/traffic 歧义当成澄清条件。
6. 不要为了简洁省略前置步骤。
...
- 高速路/高速编号 + 服务区详情/充电/配套：service -> answer
...
3. 只有在起点、终点等关键参数真正缺失且无法继续执行时，才标记 need_clarification=true；OD 问句里路线 vs 路况 的选择不要用它来追问。
4. 不要直接生成最终用户答案。
...
- 如果用户问的是“XX收费站在哪条高速 / 属于哪条高速 / 是哪个高速上的 / 在哪条路上”，这仍然属于 traffic 类问题；必须优先输出 toll_station，并尽量补全 road、road_name、road_code。不要把这类问题改成 service，也不要只输出模糊 target。
- service 元数据只用于服务区/停车区设施查询，不用于收费站归属查询；report 元数据只用于全网/全省汇总，不用于单道路或单收费站问题。
- 下面这类例子需要你自己完成归一化：
  ...
  - “佛堂收费站在哪条高速” -> traffic；toll_station=“佛堂收费站”；如果能识别所属道路，必须补齐 road、road_name、road_code，answer 阶段应直接回答所属高速。
