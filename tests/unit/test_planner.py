...
async def test_planner_can_use_llm_output(monkeypatch) -> None:
    """显式开启配置后，应允许 LLM planner 覆盖规则规划。"""

    monkeypatch.setenv("PLANNER_USE_LLM", "true")
...
async def test_planner_falls_back_to_rules_when_llm_output_is_invalid(monkeypatch) -> None:
    """LLM planner 解析失败时，应回退到规则规划。"""
